...
""" % (data['cost'], data['room_num'], data['area'], data['contacts']['phone'], data['date'], 'NULL', json.dumps(data['pics']), json.dumps(data['contacts']), data['descr'], data['adr'], json.dumps(data['metro']), data['url'], data['loc'])
        print(cmnd)
        self.db.query(cmnd)
